...
sub vcl_recv {
    ...
    if (req.http.User-Agent ~ "(?i)iPhone|iPod|BlackBerry|Pre|Palm|Googlebot-Mobile|mobi|Safari Mobile|Windows Mobile|Android|Opera Mini|mobile") {
        return (pipe);
    }
    ...
    if (req.url ~ "^(/media/|/skin/|/js/|/)(?:(?:index|litespeed)\.php/)?") {
        ...
        if (req.http.Cookie ~ "\bregionCookie=") {
            set req.http.X-Varnish-Region = regsub(
            req.http.Cookie, ".*\bregionCookie=([^;]*).*", "\1");
        }

        if (req.http.Cookie ~ "\bcustomerGroup=") {
                    set req.http.X-Varnish-Customer-Group = regsub(
                    req.http.Cookie, ".*\bcustomerGroup=([^;]*).*", "\1");
        }
        ...
        if (req.url ~ "/turpentine/esi/get(?:Block|FormKey)/") {
            ...
            req.url, ".*/access/(\w+)/.*", "\1");
            if (req.http.X-Varnish-Esi-Method == "esi" && req.esi_level == 0 && !(true || client.ip ~ debug_acl)) {
        ...
        if (true && req.url ~ ".*\.(?:css|js|jpe?g|png|gif|ico|swf|eot|svg|ttf|woff)(?=\?|&|$)") {
            unset req.http.Cookie;
            unset req.http.X-Varnish-Faked-Session;
            return (lookup);
        }

        if (req.url ~ "^(/media/|/skin/|/js/|/)(?:(?:index|litespeed)\.php/)?(?:admin|api|cron\.php|region/region|awislider_admin/adminhtml_slider|content/|catalog/product_compare|customer/account|customer/address|checkout/|decidir/|sales/|storelocator/edit|catalogsearch|import/magmi/web/)" ||
...
sub vcl_hash {
    ...
    if (
        (req.http.X-Varnish-Esi-Method == "esi" && req.http.X-Varnish-Region) ||
        (req.url ~ "/promociones"  && req.http.X-Varnish-Region)
    ) {
        hash_data("r=" + req.http.X-Varnish-Region);
    }

    if (req.http.X-Varnish-Customer-Group) {
            hash_data("g=" + req.http.X-Varnish-Customer-Group);
    }
    ...
    if (req.url ~ "^(/media/|/skin/|/js/|/)(?:(?:index|litespeed)\.php/)?") {
        ...
        if (beresp.status != 200 && beresp.status != 404) {
            ...
            if (beresp.http.X-Turpentine-Cache == "0") {
                ...
                if (true &&
                    bereq.url ~ ".*\.(?:css|js|jpe?g|png|gif|ico|swf|eot|svg|ttf|woff)(?=\?|&|$)") {
                    ...
                    if (req.http.X-Varnish-Esi-Method == "ajax" &&
                        ...
                        set beresp.http.Cache-Control = "max-age=" + regsub(req.url, ".*/ttl/(\d+)/.*", "\1");
                    }
...
sub vcl_deliver {
    ...
    if (true || client.ip ~ debug_acl) {
        ...
        set resp.http.X-Varnish-Region = req.http.X-Varnish-Region;
        set resp.http.X-Varnish-Customer-Group = req.http.X-Varnish-Customer-Group;
    } else {
